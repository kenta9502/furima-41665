class User < ApplicationRecord
  ...
  # 半角英数字が混在することを保証
  validates :first_name, presence: true, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: 'は全角の漢字・ひらがな・カタカナのみで入力してください' }
  validates :last_name,  presence: true, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: 'は全角の漢字・ひらがな・カタカナのみで入力してください' }
